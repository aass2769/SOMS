...
public class SubmissionPageController {
  ...
  public String expenseForm(Model model, HttpServletRequest request) {
    ...
    model.addAttribute("approverDto", approvers);
  ...
  @PostMapping("expense")
  public String submitv1(SubmissionDto submissionDto, @Valid ExpenseInsertForm expenseInsertForm, BindingResult expenseResult,
    ...
    for (int i = 0; i < 8; i++) {
      if (request.getParameter("approver"+i) != null && request.getParameter("approver" + i) != ""){
        approverDto.add(new ApproverDto(
            Long.parseLong(request.getParameter("approver" + i)),
    ...
    if (approverDto.size() == 0) {
      expenseResult.reject("approverIsNull", new Object[]{}, null);
    }

    //들어온 expenseDto 값들에 오류가 없는지 검증
    if (expenseResult.hasErrors()) {
      //오류 내역 로그
      log.warn("getFieldErrors={}", expenseResult.getFieldErrors());
      log.warn("getGlobalError={}", expenseResult.getGlobalError());
